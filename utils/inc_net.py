...
class CosineIncrementalNet(BaseNet):
    ...
    def weight_align(self, increment):
        weights = self.fc.fc1.weight.data
        newnorm = torch.norm(weights[-increment:, :], p=2, dim=1)
        oldnorm = torch.norm(weights[:-increment, :], p=2, dim=1)
        meannew = torch.mean(newnorm)
        meanold = torch.mean(oldnorm)
        gamma = meanold / meannew
        print("alignweights,gamma=", gamma)
        self.fc.fc1.weight.data[-increment:, :] *= gamma
...
class DERNet(nn.Module):
    ...
    def copy_fc(self, fc, nb_classes):
        weight = copy.deepcopy(fc.weight.data)
        bias = copy.deepcopy(fc.bias.data)
        n, m = weight.shape[0], weight.shape[1]
        #unet
        self.fc.weight.data = weight[:,:128]
        #resnet
        #self.fc.weight.data = weight[:,:512]
