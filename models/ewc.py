...
class EWC(BaseLearner):
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (inputs, targets) in enumerate(train_loader):
                ...
                loss_ewc = lamda * loss_ewc
                loss = loss_clf + loss_ewc + loss_kd
            ...
            if epoch % 5 == 0:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = "Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_ewc {:.3f}, Loss_kd {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    epochs,
                    losses / len(train_loader),
                ...
                info = "Task {}, Epoch {}/{} => Loss {:.3f},  Loss_clf {:.3f}, Loss_ewc {:.3f}, Loss_kd {:.3f}, Train_accy {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    epochs,
                    losses / len(train_loader),
    ...
    #     plt.legend()
    #     plt.savefig("checkpoint/tsne_ewc_"+str(self._cur_task)+".jpg")
